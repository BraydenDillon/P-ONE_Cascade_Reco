...
t_knots = np.concatenate(([-20,-10,-5,-3,-1],np.linspace(0,40,20),np.geomspace(40.1, max(t_centers), 10)))
import matplotlib.pyplot as plt
t = np.linspace(0,1400,2500)

plt.plot(t,(spline.evaluate_simple([t, 25, np.pi / 2, np.pi / 2])))
plt.plot(t_centers,pdfs[:,50,50, 39])
plt.scatter(t_knots, spline.evaluate_simple([t_knots,25,np.pi / 2, np.pi / 2]), marker='o')
...
for photon in d[:100]:
	...
	plt.title(f'Time PDF, r={truncate(photon[3], 3)}, dphi={truncate(photon[1],3)}, Ephi={truncate(photon[4], 3)}')
	plt.text(0.7, 0.8, f'CDF={truncate(np.sum(pdf[np.searchsorted(t, photon[0]):])/np.sum(pdf),5)}', transform=plt.gca().transAxes, fontsize=10)
	plt.scatter(t_knots, spline.evaluate_simple([t_knots,photon[3],photon[1], photon[4]]), marker='o')
	plt.xlabel('dt (ns)')
