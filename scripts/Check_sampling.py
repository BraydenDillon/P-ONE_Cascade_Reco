...
def compare_spline(fixed_coords: np.array, spline, frames)->None:
    ...
    hist, edges = np.histogram(pvalues, np.linspace(spline.extents[2][0], spline.extents[2][1], 500))
    #plt.plot(t, pdf, color='red')
    plt.bar(edges[:-1], hist, width=edges[1] - edges[0])
    plt.yscale('log')
    plt.savefig('/mnt/home/dillonb5/cascades/plots/sampling_cdf_2.png')
