...
def compare_spline(fixed_coords: np.array, spline, frames)->None:
    ...
    # pdf = norm.pdf(t)
    hist, edges = np.histogram(vals_in_bin, np.linspace(min(spline.knots[-1]), max(spline.knots[-1]), 500))
...
def main()->None:
    ...
    for i in range(50):
        if i < 10:
            ...
            n = '0'+str(i)
        sampled_file = dataio.I3File(f'/mnt/scratch/dillonb5/sampled_data_exact/new_exact_{n}.i3.zst')
