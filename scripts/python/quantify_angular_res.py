...
def angular_res(frame):
    if (len(frame["I3MCTree"]) != 0) and (len(frame["I3Photons"]) != 0):
        ...
        space_angle = 2 * np.arcsin(
            np.sqrt(
                np.sin((fit_zenith - truth_zenith) / 2) ** 2
                + np.cos(truth_zenith)
                * np.cos(fit_zenith)
                * np.sin((fit_azimuth - truth_azimuth) / 2) ** 2
            )
        )
...
print("Tray Finished")
np.save(
    "/mnt/scratch/dillonb5/resolutions_80str/resolution_" + runnumber + ".npy",
    angular_resolution,
)
