...
knots = [
    np.concatenate(
        (
            [-30, -20, -10, -5, -3, -1],
            np.linspace(0, 75, 20),
            np.geomspace(75.1, max(t_centers), 10),
        )
    ),
    np.concatenate(
        (
            [-30, -20, -10, -5, -3, -1],
            np.linspace(0, 60, 20),
            np.geomspace(60.1, max(r_centers), 10),
        )
    ),
...
print(resource.getrusage(resource.RUSAGE_SELF)[2])
w[~np.isfinite(w)] = np.nanmin(
    w
)  # was nanmin(w), changed to 0 to hopefully save on memory
# pdfs = np.log(pdfs)
# w = np.log(w)
sparse = photospline.ndsparse.from_data(pdfs, w)
